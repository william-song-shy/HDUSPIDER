...
class HDU:
        def __init__(self, num=1001):
                if isinstance(num, str):
                        if not num.isdecimal():
                                raise HDUError(
                                        'Num variable is not an integer'
                                        )
                self.num = num
        ...
        def work(self, headers={'User-Agent': "Mozilla/5.0 (Windows NT 6.1; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/71.0.3578.98 Safari/537.36"}):
                ...
                if response.status == 404:
                        raise HDUError(
                                'Not found error of {}'.format(self.num)
                                )
                html = response.read()
                ...
                title = re.findall(
                        r"<h1 style='color:#1A5CC8'>(.*?)</h1>",
                        html)
                ltitle = re.findall(
                        r"<div class=panel_title align=left>(.*?)</div>",
                        html)
                test = re.findall(
                        r"<div class=panel_content>([\d\D]*?)</div>",
                        html)
